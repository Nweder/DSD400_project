...
function handleAuth() {
    ...
    let endpoint = isLogin ? "/login/" : "/signup/";  // Adjust endpoint based on login/signup
...
function fetchFilteredCars() {
    ...
    fetch(`/api/get_cars${queryParams}`)
    .then(response => response.json())
    .then(data => {
        alert("✅ Cars fetched successfully!");
        console.log(data);  // Display in console for debugging
        // Populate your car options here if needed, or handle data
    })
